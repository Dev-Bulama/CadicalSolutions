...
import {
  ...
  Package, Wrench, Building2, BarChart3, MapPin,
} from "lucide-react"
...
export function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 h-16 flex items-center px-4 md:px-8 justify-between ${
      ...
      <div className="hidden lg:flex items-center gap-1 text-sm font-medium text-slate-600">
        ...
        <div className="relative" onMouseEnter={() => setProdOpen(true)} onMouseLeave={() => setProdOpen(false)}>
          <button className="flex items-center gap-1 px-3 py-2 rounded-lg hover:bg-slate-50 hover:text-[#1565C0] transition-colors">
            ...
            <ChevronDown size={12} className={`transition-transform ${prodOpen ? "rotate-180" : ""}`} />
          </button>
          {prodOpen && (
            <div className="absolute top-full left-0 mt-1 w-56 bg-white rounded-xl shadow-xl border border-slate-100 py-2 z-50">
              <div className="px-3 py-1 text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Categories</div>
              {CATEGORIES.map(c => (
                <Link key={c.name} href={c.href}
                  className="block px-3 py-2 text-sm text-slate-700 hover:bg-blue-50 hover:text-[#1565C0] transition-colors">
                  {c.name}
                </Link>
              ))}
              <div className="border-t border-slate-100 mt-1 pt-2 px-3">
                <Link href="/products" className="text-sm font-semibold text-[#1565C0] hover:underline">All products →</Link>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="relative" onMouseEnter={() => setServOpen(true)} onMouseLeave={() => setServOpen(false)}>
          <button className="flex items-center gap-1 px-3 py-2 rounded-lg hover:bg-slate-50 hover:text-[#1565C0] transition-colors">
            ...
            <ChevronDown size={12} className={`transition-transform ${servOpen ? "rotate-180" : ""}`} />
          </button>
          {servOpen && (
            <div className="absolute top-full left-0 mt-1 w-52 bg-white rounded-xl shadow-xl border border-slate-100 py-2 z-50">
              {SERVICES.map(s => (
                <Link key={s.name} href={s.href}
                  className="flex items-center gap-2 px-3 py-2 text-sm text-slate-700 hover:bg-blue-50 hover:text-[#1565C0] transition-colors">
                  <s.icon size={13} className="text-[#1565C0]" /> {s.name}
                </Link>
              ))}
            </div>
          )}
        ...
        <Link href="/institutional-portal"
          className="px-3 py-2 rounded-lg hover:bg-slate-50 hover:text-[#1565C0] transition-colors">
          Institutional
        </Link>
      </div>
      ...
      <div className="flex items-center gap-1">
        ...
        {searchOpen ? (
          <form onSubmit={handleSearch}
            ...
            <input
              ...
              className="bg-transparent text-sm outline-none w-36 text-slate-700"
            />
          ...
          <button onClick={() => setSearchOpen(true)}
            className="hidden md:flex p-2 rounded-lg hover:bg-slate-50 text-slate-500 hover:text-[#1565C0] transition-colors">
            <Search size={17} />
          </button>
        )}
        ...
        <Link href="/booking"
          className="hidden lg:inline-flex items-center gap-1.5 bg-[#1565C0] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#0d47a1] transition-colors ml-1">
          Book Service
        </Link>

        {/* Hamburger */}
        <button
          className="lg:hidden p-2 rounded-lg hover:bg-slate-50 text-slate-600 ml-1"
      ...
      {open && (
        <div className="absolute top-16 left-0 right-0 bg-white border-b border-slate-100 shadow-xl lg:hidden z-50 overflow-y-auto max-h-[calc(100vh-4rem)]">

          {/* Search bar */}
          <form onSubmit={handleSearch} className="mx-4 mt-4 mb-2 flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5">
          ...
          <div className="px-4 pb-2">
            {MOBILE_SECTIONS.map(section => (
              <div key={section.label} className="border-b border-slate-100 last:border-none">
                <button
                  onClick={() => toggleExpanded(section.label)}
                  className="w-full flex items-center justify-between py-3 text-sm font-semibold text-slate-800"
                >
                  <span className="flex items-center gap-2">
                    <section.icon size={15} className="text-[#1565C0]" />
                    {section.label}
                  </span>
                  <ChevronDown
                    size={14}
                    className={`text-slate-400 transition-transform duration-200 ${expanded === section.label ? "rotate-180" : ""}`}
                  />
                </button>

                {expanded === section.label && (
                  <div className="pb-2 pl-6 space-y-0">
                    {section.links.map(link => (
                      <Link
                        key={link.label}
                        href={link.href}
                        onClick={() => setOpen(false)}
                        className="flex items-center gap-1.5 py-2 text-sm text-slate-600 hover:text-[#1565C0] transition-colors"
                      >
          ...
          {/* Bottom CTAs */}
          <div className="px-4 py-4 flex flex-col gap-2.5 border-t border-slate-100">
            <Link href="/booking" onClick={() => setOpen(false)}
              className="bg-[#1565C0] text-white px-4 py-2.5 rounded-xl text-sm font-semibold text-center">
              Book a Service
            </Link>
            <Link href="/auth/login" onClick={() => setOpen(false)}
              className="border border-slate-200 text-slate-700 px-4 py-2.5 rounded-xl text-sm font-semibold text-center">
              Sign In
            </Link>
          </div>
        </div>
