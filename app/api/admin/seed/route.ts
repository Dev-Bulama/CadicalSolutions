...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    log.push(`Products: ${PRODUCTS.length} seeded`)
    ...
    if (!existingProfile) {
      ...
      log.push("Technician profile created")
    }
    ...
    if (!existingSupplier) {
      ...
      log.push("Supplier profile created")
    }
    ...
    if (products.length > 0) {
      ...
      log.push("Sample orders created")
    }
    ...
    if (institutionId) {
      ...
      if (!existing) {
        ...
        log.push("Maintenance schedule created")
      }
    }
    ...
    if (!existingBooking && techProfile) {
      ...
      log.push("Service booking created")
    }
    ...
    await prisma.notification.createMany({
      skipDuplicates: true,
      data: [
        { userId: adminId, type: "SYSTEM",     title: "New Supplier Registration", message: "MedTech Supply Nigeria Ltd has submitted KYC documents for review.", actionUrl: "/admin/suppliers/pending", isRead: false },
        { userId: adminId, type: "JOB_UPDATE", title: "Service Job Completed",     message: "Technician Emeka Okafor has completed the repair on Philips MX800.", actionUrl: "/admin/service-jobs",      isRead: true  },
        { userId: adminId, type: "SYSTEM",     title: "Maintenance Overdue Alert", message: "GE Voluson E10 at Lagos General Hospital is overdue for quarterly maintenance.", actionUrl: "/admin/maintenance", isRead: false },
      ],
    })
    ...
    await prisma.auditLog.createMany({
      data: [
        ...
        { userId: createdUsers["admin"],      userEmail: "admin@cadical.com",      userRole: "admin",      action: "approve", entity: "supplier",                                       ipAddress: "105.113.22.8" },
